-- 修復簽到進度表的 updated_at trigger 函數
-- 問題：trigger 函數使用了錯誤的欄位名稱 updatedAt 而不是 updated_at

-- 刪除所有相關的舊 trigger
DROP TRIGGER IF EXISTS update_checkin_progress_updated_at ON checkin_progress;
DROP TRIGGER IF EXISTS update_checkin_activity_updated_at ON checkin_activity;

-- 刪除可能存在的錯誤函數
DROP FUNCTION IF EXISTS update_updated_at_column() CASCADE;

-- 創建正確的 trigger 函數，使用正確的欄位名稱 updated_at
CREATE OR REPLACE FUNCTION update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = CURRENT_TIMESTAMP;
    RETURN NEW;
END;
$$ LANGUAGE 'plpgsql';

-- 為簽到進度表創建 trigger
CREATE TRIGGER update_checkin_progress_updated_at
    BEFORE UPDATE ON checkin_progress
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- 為簽到活動表創建 trigger
CREATE TRIGGER update_checkin_activity_updated_at
    BEFORE UPDATE ON checkin_activity
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- 確認 trigger 已創建
SELECT 
    t.tgname as trigger_name,
    c.relname as table_name,
    p.proname as function_name
FROM pg_trigger t
JOIN pg_class c ON c.oid = t.tgrelid
JOIN pg_proc p ON p.oid = t.tgfoid
WHERE c.relname IN ('checkin_progress', 'checkin_activity')
ORDER BY c.relname, t.tgname;

-- 確認欄位存在
SELECT 
    table_name,
    column_name,
    data_type
FROM information_schema.columns 
WHERE table_name IN ('checkin_progress', 'checkin_activity')
    AND column_name = 'updated_at'
ORDER BY table_name;